...
--
-- Host: localhost    Database: camrstidb-test
-- ------------------------------------------------------
...
INSERT INTO camrstidb.SampleInfo
(id, `type`, source, `year`, people, imageId, `describe`, `explain`, experimentId)
VALUES('11Y1:9', '炉壁', '北京延庆水泉沟', 2019, '刘海峰', '["IMG_20190512_33333.jpg"]', '取自炉体上半部分，红色，质地疏松多空', '自内向外取三处，分别做薄片后矿相观察，制备粉末样品开展XRD分析', '["11Y3:9-1", "11Y3:9-2", "11Y3:9-3"]');
...
CREATE TABLE `MicroView` (
	...
	`sampleImage` json NULL COMMENT '样品全图',
	`sampleDescribe` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL COMMENT '描述',
	`device` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL COMMENT '设备',
	`imageData` json NULL COMMENT '照片数据（放大倍数、拍摄模式、照片）',
	`sampleId` varchar(30) NULL COMMENT '样品id',
	CONSTRAINT NewTable_PK PRIMARY KEY (id),
	CONSTRAINT NewTable_FK FOREIGN KEY (sampleId) REFERENCES camrstidb.SampleInfo(id) ON UPDATE CASCADE
)
...
CREATE TABLE `MineContentInfo` (
  `id` varchar(30) COLLATE utf8mb4_general_ci NOT NULL COMMENT '样品号',
  `sampleName` varchar(50) COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT '样品名称',
...
CREATE TABLE `MineThermalInfo` (
  ...
  `surveImage` varchar(100) COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT '热分析曲线',
  PRIMARY KEY (`id`)
...
CREATE TABLE `MineXRDInfo` (
  `id` varchar(30) COLLATE utf8mb4_general_ci NOT NULL COMMENT '样品编号',
  `type` varchar(40) COLLATE utf8mb4_general_ci DEFAULT NULL COMMENT '类型',
  `quartz` int DEFAULT NULL COMMENT '石英',
  `albite` int DEFAULT NULL COMMENT '钠长石',
  `potashFeldspar` int DEFAULT NULL COMMENT '钾长石',
  `mica` int DEFAULT NULL COMMENT '云母',
  `amphibole` int DEFAULT NULL COMMENT '闪石',
  `hematite` int DEFAULT NULL COMMENT '赤铁矿',
  `magnetite` int DEFAULT NULL COMMENT '磁铁矿',
  `dolomite` int DEFAULT NULL COMMENT '白云石',
  `analcite` int DEFAULT NULL COMMENT '方沸石',
  `tridymite` int DEFAULT NULL COMMENT '磷石英',
  `cristobalite` int DEFAULT NULL COMMENT '方石英',
  `mullite` int DEFAULT NULL COMMENT '莫来石',
  PRIMARY KEY (`id`)
